...
SELECT ScriptName FROM AppliedMigrationScript";
    ...
    public async Task<bool> UpgradeDatabaseAsync(string scriptPath) {
        using var conn = connectionManager.CreateConnection();
        var appliedScriptNames = (await conn.QueryAsync<string>(APPLIED_MIGRATION_SCRIPT_SQL).ConfigureAwait(false)).ToList();
        var migrationScripts = await getScriptsAsync(scriptPath, appliedScriptNames).ConfigureAwait(false);
